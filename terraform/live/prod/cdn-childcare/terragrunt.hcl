...
dependency "dns_childcare" {
  config_path = "../dns-childcare"
...
dependency "compute" {
  ...
  mock_outputs = {
    ...
    api_key_ssm_param = "/beststartinlife/prod/api-key"
  }
  mock_outputs_allowed_terraform_commands = ["validate", "plan"]
}
...
inputs = {
  ...
  domain_name             = "childcare.beststartinlife.gov.uk"
  certificate_arn         = dependency.dns_childcare.outputs.certificate_arn
  route53_zone_id         = dependency.dns_childcare.outputs.zone_id
  ...
  basic_auth_associated     = false
  basic_auth_ssm_user_param = "/beststartinlife/prod/basic-auth/user"
  basic_auth_ssm_pass_param = "/beststartinlife/prod/basic-auth/pass"
  name_suffix               = "childcare"
